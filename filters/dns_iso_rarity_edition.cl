...
//
// ISOLATING fixture.  Runs ONLY the two shop joker staging polls --
// R_Joker_Rarity/<ante>/S_Shop and R_Joker_Edition/S_Shop/<ante> -- one draw
// each per joker slot, inside the real DNS_CHUNK staging loop.  Standalone
// cost of the rarity+edition pair; compare with diag_dns_abl_rarity.cl /
// diag_dns_abl_edition.cl for their marginal costs.
//
// Stripped: card-type poll, all identity draws and resample chains, packs,
// tags.  Never wire this into tests/golden/.
//
// VOLUME FIDELITY -- how it was achieved:
//   The real ante loop, the real dns_frames(ante) and the real frame-size
//   rules give `cards` exactly as the filter computes it.  The number of joker
//   slots then has to come from somewhere without running the card-type poll
//   (which is 58% of the filter's draws and would destroy the isolation), so
//   it uses the exact expected joker count  jokerCards = (cards * 20) / 28  --
//   the shop's joker rate is 20 of a total rate of 28 -- computed PER ANTE
//   from the real `cards`, which is the same stand-in diag_dns_abl_cardtype.cl
//   already uses.  So the volume tracks the real frame schedule per ante and
//   per seed; only the binomial jitter around the 20/28 mean is removed.
//   The DNS_CHUNK staging loop is kept so the loop nest matches.
//   Draws: jokerCards on each of the two nodes.
//
//   MEASURED (DNS_ISO_COUNT build, 2,048 seeds from 11111111):
//          rarity node  mean 13,427  range 12,777..14,959  (target 13,432, -0.04%)
//          edition node mean 13,427  (identical by construction: one of each per slot)
//          Cross-check: dns_iso_cardtype.cl, running the REAL card-type poll on
//          the same 2,048 seeds, reports mean 13,444 joker slots. The 20/28
//          stand-in is therefore 0.13% low -- within the binomial noise it
//          replaces (sd ~57 per seed).
//
...
long filter(instance* inst) {
    ...
    for (int ante = 1; ante <= DNS_LAST_ANTE; ante++) {
        ...
        // Exact expected joker count: shop joker rate 20 of total rate 28.
        ...
        if (jokerCards > 0) {
            ...
            for (int base = 0; base < jokerCards; base += DNS_CHUNK) {
                ...
                int uncommonCount = 0, rareCount = 0, negCount = 0;
                for (int slot = 0; slot < chunkSize; slot++) {
                    ...
                    // Both poll results stay live; no identity is drawn.
                    if (r == Rarity_Common) c.other += negative;
                    else if (r == Rarity_Uncommon) { uncommonCount++; negCount += negative; }
                    else { rareCount++; negCount += negative; }
                }
                c.uncommon += uncommonCount;
                c.copy += rareCount + negCount;
            }
            ISO_TALLY(0, jokerCards);   // rarity-node draws
            ISO_TALLY(1, jokerCards);   // edition-node draws
