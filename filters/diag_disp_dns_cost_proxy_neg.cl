...
//
// NEGATED VARIANT: returns minus the predicted shop-card count, so that
// `-c -17910` selects exactly the seeds sitting at the modal minimum cost
// (no early Overstock at all, ~35.5% of seeds) and yields a cost-uniform pool.
// Returns minus the predicted shop-card count. Used to bucket a seed pool by cost so
