...
// Ablates the PACK loop (next_pack + buffoon contents). Side-effect free:
// buffoon locks are taken and released within the loop, and next_pack's
// first-pack special case never fires for ante >= 3.
// ===========================================================================
...
long filter(instance* inst) {
    ...
    for (int ante = 1; ante <= DNS_LAST_ANTE; ante++) {
        ...
        inst->rng = shopRng;
        for (int p = 0; p < 0; p++) { // ABLATED pack stream
