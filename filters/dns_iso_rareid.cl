...
//
// ISOLATING fixture.  Runs ONLY the shop RARE identity stream: the base draw on
// R_Joker_Rare/S_Shop/<ante> plus its full lock-resample chain on
// R_Joker_Rare/S_Shop/<ante>/resample_<d>.  Standalone cost of the stream
// including its divergence tax; diag_dns_abl_rareid.cl gives the marginal cost.
// This is the most divergent stream in the filter: 11 of 20 rares are locked,
// so the mean chain is 1.22 resamples per base draw and its tail is long.
//
// Stripped: card-type, rarity and edition polls, the uncommon identity pool,
// packs, tags.  Never wire this into tests/golden/.
//
// VOLUME FIDELITY -- how it was achieved:
//   Real ante loop, real dns_frames(ante), real frame-size rules -> real
//   `cards`; then jokerCards = (cards * 20) / 28 (the exact expected joker
//   count, as diag_dns_abl_cardtype.cl uses) stands in for the card-type poll,
//   and the rare slot count is taken as a RUNNING QUOTA over the chunked slot
//   index, rounded to nearest --
//   ((base + chunkSize + 10) / 20) - ((base + 10) / 20) -- which is exactly the
//   5% the rarity poll produces (poll > 0.95).  Running-quota rather than
//   per-chunk rounding keeps the per-ante total exact and the per-flush counts
//   the real size (DNS_CHUNK / 20 per flush -- 25 or 26 at the DNS_CHUNK of 512
//   this fixture inherits from the filter), which matters here: a flush of n
//   rares is what the depth-major resample loop amortises over, and DNS_CHUNK
//   is exactly the knob diag_dns_chunk*.cl is sweeping.  Because the constant
//   is taken from the filter's own prelude rather than restated, this fixture
//   tracks that sweep automatically.
//   Rounding to nearest rather than flooring matters more here than anywhere
//   else in the family: this stream draws only ~18 per ante, so a floor cost it
//   2% of its whole volume (measured over 2,048 seeds: 657 -> 673).  O(1) per
//   chunk, so no
//   rarity-stream per-slot loop leaks in.
//   The identity stream's cost IS its divergence, so the real machinery is kept
//   intact: the real DNS_CHUNK chunking, the real dns_flush_identities() from
//   the prelude (taken verbatim -- the only delta against
//   deep_negative_shops.cl is three #ifdef DNS_ISO_COUNT tally hooks that
//   compile out of the measured build), the real depth-major
//   `for (int depth = 1; dnsm_any(&locked); depth++)` resample chain with its
//   data-dependent trip count, and the real per-pool locked bitmask from the
//   lock lists.  Nothing about the trip counts is flattened -- the resample
//   depth distribution a lane sees here is the one it sees in the real filter.
//   Draws: base     = sum over antes of round(jokerCards / 20)
//          resample = geometric on p_locked = 11/20, so 11/9 = 1.222 per base
//                     draw in expectation.
//
//   MEASURED (DNS_ISO_COUNT build, 2,048 seeds from 11111111):
//          base     mean 673  range 641..748     (target 671, +0.3%)
//          resample mean 822  range 624..1,034   (target 820, +0.24%)
//          resample / base = 1.221, exactly 11/9 -- the check that the locked
//          bitmask and the resample chain are real.  Note the resample range is
//          +-25% around its mean against +-8% for the base draws: that spread
//          is the divergence this fixture exists to price.
//
//   The Negative-edition mask is staged empty: the edition poll belongs to
//   dns_iso_rarity_edition.cl and those bits change no draw, only the final
//   two popcounts of a flush.
//
//   RETAINED DEPENDENCY: the voucher stream, for frameSize -- see
//   dns_iso_cardtype.cl for the full rationale and the double-count caveat.
// ===========================================================================
...
#ifdef DNS_ISO_COUNT
    , long* isoDraws   // [0] base-node draws, [1] resample-node draws
#endif
) {
...
long filter(instance* inst) {
    ...
    for (int ante = 1; ante <= DNS_LAST_ANTE; ante++) {
        ...
        // Verbatim frame schedule. frameSize and dns_frames(ante) are what make
        // the per-ante volume identical to the real filter's.
        int frameSize = 2;
        if (overstock || ante >= 12) frameSize = 3;
        if (overstockPlus || ante >= 24) frameSize = 4;
        int cards = dns_frames(ante) * frameSize;
        int jokerCards = (cards * 20) / 28;
        if (jokerCards > 0) {
            lrandom shopRng;
            rng_node_id shopRareNode = RNG_NODE_INVALID;
            double shopRareState = 0;

            for (int base = 0; base < jokerCards; base += DNS_CHUNK) {
                int chunkSize = min(DNS_CHUNK, jokerCards - base);
                // Exactly 5% of slots, as a running quota over the slot index,
                // rounded to nearest (the +10 bias) so the ante total is
                // round(jokerCards / 20).  With a plain floor this stream lost
                // half a draw per ante -- 2% of its whole volume, because it
                // only draws ~18 of them in an ante.
                int rareCount = ((base + chunkSize + 10) / 20) - ((base + 10) / 20);
                dns_mask rareNegative;
                dnsm_clear(&rareNegative);
