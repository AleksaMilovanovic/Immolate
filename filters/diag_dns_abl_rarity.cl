...
// Ablates the shop RARITY stream: the poll is replaced by a deterministic
// 70/25/5 cycle over the slot ordinal, preserving the downstream mix of
// common / uncommon / rare identity draws. Time drop = rarity share.
// ===========================================================================
        ...
        if (jokerCards > 0) {
            ...
            for (int base = 0; base < jokerCards; base += 32) {
                ...
                for (int slot = 0; slot < chunkSize; slot++) {
                    int _q = (base + slot) % 20; // ABLATED rarity stream
                    rarity r = (_q == 0) ? Rarity_Rare
                             : (_q <= 5) ? Rarity_Uncommon : Rarity_Common;
