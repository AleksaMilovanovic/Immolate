...
// DIVERGENCE PROBE.
//
// WHAT IS FORCED UNIFORM: jokerCards, the trip count of the rarity/edition
// staging loop and hence the number of DNS_CHUNK chunks and the size of
// every identity flush. It becomes cards * 5/7 instead of the seed's actual
// joker count, so given the same frame size every lane stages the same
// number of slots and flushes at the same boundaries.
//
// CONSTANT: 5/7 = 0.714286, the shop joker share (rates 20/4/4/0/0, total
// 28). Derived and cross-checked against measured draws in the body comment.
// Per-seed joker cards land at 18,796 * 5/7 = 13,426, which is the measured
// mean, so total draws are held to ~52,800.
//
// Note this does NOT make the loop uniform across seeds on its own: `cards`
// still varies with Overstock. Pair it with dns_div_frames_uniform to remove
// that second source, or read this one as isolating the card-type poll's
// contribution to staging-loop divergence.
//
// PREDICTION: 0.85x-1.00x. The staging chunk count already quantises this
// (jokerCards varies by a few percent but ceil(jokerCards/128) usually does
// not), so most of the win should be in the flush sizes rather than the chunk
// count. A ratio well below 0.85x would say per-lane joker-count spread is a
// first-order cost and DNS_CHUNK should be raised further - consistent with
// the chunk sweep not having saturated at 512 (0.859x).
// =========================================================================
...
long filter(instance* inst) {
    ...
    for (int ante = 1; ante <= DNS_LAST_ANTE; ante++) {
        ...
        // DIVERGENCE PROBE: the card-type poll still runs for every card (same
        // draws, same get_item_type work - the volatile sink keeps it from
        // being optimised away), but its result no longer steers the staging
        // loop. jokerCards is a lane-uniform function of `cards`.
        //
        // Constant: the shop card-type rates are fixed for this filter -
        // get_shop_instance is called once before any voucher is activated, so
        // jokerRate 20, tarotRate 4, planetRate 4, playingCard 0, spectral 0,
        // totalRate 28, joker share 20/28 = 5/7 = 0.714286.
        // Confirmed against the measured per-seed draw counts: 3,358 uncommon
        // base draws / 0.25 = 13,432 joker cards and 671 rare / 0.05 = 13,420,
        // mean 13,426; over the measured 18,796 shop cards that is 13,432/18,796
        // = 0.71462 and 13,420/18,796 = 0.71398, bracketing 5/7 within 0.1%.
        int jokerObserved = 0;
        for (int i = 0; i < cards; i++) {
            double card_type = dns_random_scalar(inst,
                &cardTypeState, &shopRng) * totalRate;
            jokerObserved += get_item_type(shopInstance, card_type) == ItemType_Joker;
        }
        dnsDivSink = jokerObserved;
        int jokerCards = (cards * 5) / 7;
