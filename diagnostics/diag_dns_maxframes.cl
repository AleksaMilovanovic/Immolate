...
// DIVERGENCE PROBE. Frame size is pinned to the largest value any seed could
// reach at that ante (3 below ante 12, 4 from ante 12), so every lane does
// 20,211 cards: 7.5% MORE lane work than the stock filter's 18,806 mean,
// but with zero frame-size divergence. If this is not slower than the
// baseline, warp divergence dominates ALU work on this device.
// ===========================================================================
...
long filter(instance* inst) {
    ...
    for (int ante = 1; ante <= DNS_LAST_ANTE; ante++) {
        ...
        int frameSize = 3; // DIVERGENCE PROBE: worst-case uniform frames
        if (ante >= 12) frameSize = 4;
