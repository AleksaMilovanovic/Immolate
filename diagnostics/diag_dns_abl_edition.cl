...
// Ablates the shop EDITION stream: the negative poll is replaced by a
// deterministic 1-in-333 cycle (same ~0.3% negative rate, same downstream
// branch mix). Time drop = edition share.
// ===========================================================================
        ...
        if (jokerCards > 0) {
            ...
            for (int base = 0; base < jokerCards; base += 32) {
                ...
                for (int slot = 0; slot < chunkSize; slot++) {
                    ...
                    bool negative = ((base + slot) % 333) == 0; // ABLATED edition stream
