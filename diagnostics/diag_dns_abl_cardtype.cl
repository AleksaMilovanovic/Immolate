...
// Ablates the shop CARD-TYPE stream: the per-card poll is replaced by the
// exact expected joker count (cards * 20 / 28), so every downstream stream
// still runs the same number of times. Time drop = card-type share.
// ===========================================================================
...
long filter(instance* inst) {
    ...
    for (int ante = 1; ante <= DNS_LAST_ANTE; ante++) {
        ...
        int jokerCards = (cards * 20) / 28; // ABLATED card-type stream
        shopRng.state = (ulong4)(1, 2, 3, 4); shopRng.out.ul = 0;
