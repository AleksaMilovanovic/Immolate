...
// Ablates the shop UNCOMMON IDENTITY stream (base draw + its resample loop).
// Everything else, including the count of uncommon slots, is untouched.
// Time drop = uncommon-identity share INCLUDING its divergence tax.
// ===========================================================================
        ...
        if (jokerCards > 0) {
            ...
            for (int base = 0; base < jokerCards; base += 32) {
                ...
                for (int ordinal = 0; ordinal < uncommonCount; ordinal++) {
                    int jokerIndex = -1; // ABLATED uncommon identity stream
