...
- `-g`: Sets the number of work-groups launched by the searcher. The work-group size is chosen from the device automatically (32 or 64 work-items). If omitted, Immolate launches 8 work-groups per compute unit, which is usually enough to fill a GPU; try larger values if you want to experiment. Use `-g 1` together with `-n 1` for single-seed analysis.
