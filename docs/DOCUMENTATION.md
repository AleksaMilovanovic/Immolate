...
`--from` also accepts a **plain-text seed list**, so a filter's printed output can be fed straight into the next filter without converting it first:

```
immolate -f deep_negative_shops -c 1000000 -n 400 > output.txt
immolate -f analyze_naneinf_negatives --from output.txt -c 1
```

A line is read as a seed only if it is exactly one seed (`1111112U`) or exactly a printed score line (`1111112U (420)`). Anything else is counted and ignored, which is what lets a redirected run be used as-is: Immolate's own banner lines go into the same file, and several of them (`Done`, `Starting`) are themselves valid base-35 seed strings, so a looser rule would silently search them. Blank lines and `#` comments are skipped, the list is sorted and deduplicated, and `-n` caps it as with a supplier file. The format is detected from the file's contents, so no extra flag is needed. Supplier files stay the better choice for large pools -- they are far smaller and are read in chunks rather than held in memory -- but a text list is the convenient thing for a few thousand hand-picked seeds.
